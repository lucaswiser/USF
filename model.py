    ...
    def __init__(self, config):
        ...
        with tf.variable_scope("rnn1", reuse=None):
            ...
            if keep_prob1 < 1:
                #Only on the inputs for rnn1. That way we don't dropout twice 
                lstm_cell_1 = rnn_cell.DropoutWrapper(
                  lstm_cell_1, output_keep_prob=keep_prob1)
                lstm_back_cell_1 = rnn_cell.DropoutWrapper(
                  lstm_back_cell_1, output_keep_prob=keep_prob1)
        ...
        #tok_embeds = outputs1[-1]
        tok_embeds = tf.concat(1, [last_fw_state1, last_bw_state1])
        _sliced = tf.dynamic_partition(tok_embeds, list(range(sent_len))*batch_size, sent_len)
        #We want a 3 dim matrix of [batch, time, hidden_size]
        #Right now we have a list of [batch, hiddden] of len time
        conv_input = tf.pack(_sliced)
        #Now its [time, batch, hidden]
        conv_input = tf.transpose(conv_input, [1,0,2])
        ...
        for i,filter_size in enumerate(filter_sizes):
            with tf.name_scope("conv-maxpool-%s" %(filter_size)):
                filter_shape = [filter_size, state_size1*4, 1, num_filters[i]]
                W = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.1, name="W"))
                b = tf.Variable(tf.constant(0.1, shape=[num_filters[i]]), name="b")
                conv = tf.nn.conv2d(
                    conv_input,
        ...
        h_pool = tf.concat(3, pooled_outputs)
        h_pool_flat = tf.reshape(h_pool, [-1, output_size])
        conv_output = tf.nn.dropout(h_pool_flat, config.keep_prob2)
